...
# warning: the next line might indicate a real incompatibility with newer sqlalchemy
# pylint: disable=arguments-differ,signature-differs,method-hidden
...
class PG13Dialect(sqlalchemy.engine.default.DefaultDialect):
  ...
  def _get_default_schema_name(self, connection):
    return 'default' # hmm; I don't support schemas, do I?
  ...
  def get_table_names(self, connection, schema=None, **kw):
    return list(connection.connection.connection.db.keys())
  ...
  def get_columns(self, connection, table_name, schema=None, **kw):
    return connection.pool.tables[table_name].fields

  def _get_column_info(self, name, format_type, default, notnull, schema):
    raise NotImplementedError

  def get_pk_constraint(self, connection, table_name, schema=None, **kw):
    return connection.pool.tables[table_name].pkey

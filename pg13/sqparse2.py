...
# pylint: disable=invalid-name,no-self-use,super-init-not-called
...
class BaseX(treepath.PathTree):
  ...
  def __init__(self, *args):
    if len(args) != len(self.ATTRS):
      raise TypeError('wrong_n_args', len(args), len(self.ATTRS), args)
...
class Literal(BaseX):
  ATTRS = ('val',)
  def toliteral(self):
    return self.val # pylint: disable=no-member
...
class JoinTypeX(BaseX):
  ...
  @property
  def inner(self):
    return not self.is_outer # pylint: disable=no-member

class JoinX(BaseX):
  ATTRS = ('a', 'b', 'on_stmt', 'jointype')
...
class OpX(BaseX):
  ...
  def __init__(self, op):
    self.op = op
    if op not in self.PRIORITY:
      raise SQLSyntaxError('unk_op', op)
  def __lt__(self, other):
    "this is for order of operations"
    if not isinstance(other, OpX):
      raise TypeError
...
class CommaX(BaseX):
  ...
  def __init__(self, children):
    self.children = list(children) # needs to be a list (i.e. mutable) for SubLit substitution (in sqex.sub_slots)

  def __iter__(self):
    raise NotImplementedError("don't iterate CommaX directly -- loop on x.children")

class CallX(BaseX):
  ATTRS = ('f', 'args') # args is not VARLEN; it's a commax because it's passed to sqex.Evaluator I think
...
class TypeX(BaseX):
  ATTRS = ('type', 'width') # width is e.g. 20 for VARCHAR(20), null for e.g. TEXT.

class CastX(BaseX):
  ATTRS = ('expr', 'to_type')

class CommandX(BaseX):
  "base class for top-level commands. probably won't ever be used."
...
class ColX(BaseX):
  ATTRS = ('name', 'coltp', 'isarray', 'not_null', 'default', 'pkey')

class PKeyX(BaseX):
  ATTRS = ('fields',)
  VARLEN = ('fields',)

class TableConstraintX(BaseX):
  "intermediate base class for table constraints. PKeyX isn't included in this because it has its own slot in CreateX."

class CheckX(TableConstraintX):
  ATTRS = ('expr',)
...
class DropX(CommandX):
  ATTRS = ('ifexists', 'name', 'cascade')
...
class DeleteX(CommandX):
  ATTRS = ('table', 'where', 'returnx')

class StartX(CommandX):
  ATTRS = ()

class CommitX(CommandX):
  ATTRS = ()

class RollbackX(CommandX):
  ATTRS = ()
...
def bin_priority(op, left, right):
  ...
  if isinstance(left, BinX) and left.op < op:
    return bin_priority(left.op, left.left, bin_priority(op, left.right, right))
  elif isinstance(left, UnX) and left.op < op:
    return un_priority(left.op, BinX(op, left.val, right)) # note: obviously, no need to do this when right is a UnX
  elif isinstance(right, BinX) and right.op < op:
    return bin_priority(right.op, bin_priority(op, left, right.left), right.right)
  else:
...
class SqlGrammar:
  ...
  def t_error(self, t):
    raise SQLSyntaxError(t) # I think t is LexToken(error,unparsed_tail)
  ...
  def p_float(self, t):
    ...
    t[0] = Literal(float('%s.%s' % (t[1], t[3])))
  def p_int(self, t):
    "expression : INTLIT"
    t[0] = Literal(int(t[1]))
  def p_strlit(self, t):
    "expression : STRLIT"
    t[0] = Literal(t[1][1:-1].replace("\\'", "'")) # warning: this is not safe
  def p_asterx(self, t):
    "expression : '*'"
    t[0] = AsterX()
  def p_null(self, t):
  ...
  def p_castx(self, t):
    "expression : expression CAST typename"
    t[0] = CastX(t[1], t[3])
  def p_castx2(self, t):
  ...
  def p_fromlist(self, t):
    "fromlist : kw_from fromitem_list \n | "
    t[0] = t[2] if len(t) == 3 else []
  def p_wherex(self, t):
    "wherex : kw_where expression \n | "
    t[0] = t[2] if len(t) == 3 else None
  def p_order(self, t):
    "order : kw_order kw_by expression \n | "
    t[0] = t[3] if len(t) == 4 else None
  def p_limit(self, t):
    "limit : kw_limit expression \n | "
    t[0] = t[2] if len(t) == 3 else None
  def p_offset(self, t):
    "offset : kw_offset expression \n | "
    t[0] = t[2] if len(t) == 3 else None
  def p_group(self, t):
    "group : kw_group kw_by expression \n | "
    t[0] = t[3] if len(t) == 4 else None
  def p_selectx(self, t):
    "selectx : kw_select commalist fromlist wherex group order limit offset"
    t[0] = SelectX(*t[2:])
  def p_extra_x(self, t):
    "expression : selectx \n | aliasx"
    t[0] = t[1] # expressions that also need to be separately addressable
  def p_isarray(self, t):
    "is_array : '[' ']' \n | "
    t[0] = len(t) > 1
  def p_isnotnull(self, t):
    "is_notnull : kw_not kw_null \n | "
    t[0] = len(t) > 1
  def p_default(self, t):
    "default : kw_default expression \n | "
    t[0] = t[2] if len(t) > 1 else None
  def p_ispkey(self, t):
    "is_pkey : kw_primary kw_key \n | "
    t[0] = len(t) > 1
  ...
  def p_namelist(self, t):
    ...
    if len(t) == 2:
      ...
      raise NotImplementedError('unk_len', len(t)) # pragma: no cover
  def p_pkey(self, t):
    "pkey_stmt : kw_primary kw_key '(' namelist ')'"
    t[0] = PKeyX(t[4])
  def p_nexists(self, t):
    "nexists : kw_if kw_not kw_exists \n | "
    t[0] = len(t) > 1
  ...
  def p_createx(self, t):
    ...
    if len(t) == 6:
      ...
      all_constraints = {k: list(group) for k, group in itertools.groupby(t[6], type)}
  ...
  def p_ifexists(self, t):
    "ifexists : kw_if kw_exists \n | "
    t[0] = len(t) > 1
  def p_cascade(self, t):
    "cascade : kw_cascade \n | "
    t[0] = len(t) > 1
  def p_dropx(self, t):
  ...
  def p_optparennamelist(self, t):
    "opt_paren_namelist : '(' namelist ')' \n | "
    t[0] = t[2] if len(t) > 1 else None
  ...
  def p_startx(self, t):
    "expression : kw_start kw_transaction"
    t[0] = StartX()

  def p_commitx(self, t):
    "expression : kw_commit"
    t[0] = CommitX()

  def p_rollbackx(self, t):
    "expression : kw_rollback"
    t[0] = RollbackX()

  def p_error(self, t):
    raise SQLSyntaxError(t)

...
class ColumnNameError(Exception):
  "name not in any tables or name matches too many tables"
class TableNameError(Exception):
  "expression referencing unk table"

# todo doc: explain why it's not necessary to do these checks on the whereclause
def consumes_rows(ex):
  return isinstance(ex, sqparse2.CallX) and ex.f in ('min', 'max', 'count')
def returns_rows(ex):
  return isinstance(ex, sqparse2.CallX) and ex.f in ('unnest', )
def contains(expr, field):
  return bool(treepath.sub_slots(expr, field, match=True))

# pylint: disable=too-many-return-statements, too-many-branches
def evalop(oper, left, right):
  "this takes evaluated left and right (i.e. values not expressions)"
  if oper in ('=', ' != ', '>', '<'):
    return threevl.ThreeVL.compare(oper, left, right)
  elif oper in ('+', '-', '*', '/'):
    # todo: does arithmetic require threevl?
    if oper == '/':
      raise NotImplementedError('todo: spec about int/float division')
    return (left + right) if oper == '+' else (left - right) if oper == '-' else (left * right) if oper == '*' else (left/right)
  elif oper == 'in':
    return (tuple(left) in right) if isinstance(left, list) and isinstance(right[0], tuple) else (left in right)
  elif oper in ('and', 'or'):
    return threevl.ThreeVL.andor(oper, left, right)
  elif oper in ('is not', 'is'):
    if right is not None:
      raise NotImplementedError('can null be on either side? what if neither value is null?')
    return (left is not None) if (oper == 'is not') else (left is None)
  elif oper == '@>':
    # todo: support a TextSearchDoc that will overload a lot of these operators
    if not all(isinstance(x, list) for x in (left, right)):
      raise TypeError('non-array-args', oper, left, right)
    ...
    if not all(isinstance(x, set) for x in (left, right)):
      raise TypeError('non_set_args', oper, type(left), type(right))
    return bool(left & right)
  else:
    raise NotImplementedError(oper, left, right) # pragma: no cover
...
def infer_columns(selectx, tables_dict):
  ...
  todo: I think there's common logic here and inside NameIndexer that can be merged.
  todo: this is a beast
  """
  # todo: support CTEs -- with all this plumbing, might as well
  table2fields = {}
  table_order = []
  for table in selectx.tables:
    if isinstance(table, str):
      table2fields[table] = tables_dict[table].fields
      table_order.append(table)
    elif isinstance(table, sqparse2.AliasX):
      if isinstance(table.name, str):
        table2fields[table] = tables_dict[table]
        table_order.append(table.name)
      elif isinstance(table.name, sqparse2.SelectX):
        raise NotImplementedError('todo: inner subquery')
      else:
        raise TypeError('AliasX.name', type(table.name))
    else:
      raise TypeError('table', type(table))
  ...
  for col in selectx.cols.children:
    if isinstance(col, sqparse2.AsterX):
      for table in table_order:
        cols.extend(table2fields[table])
      ...
      for path in paths:
        item = col[path]
        if isinstance(item, sqparse2.AttrX):
          if not isinstance(item.parent, sqparse2.NameX):
            raise TypeError('parent_not_name', type(item.parent))
          if isinstance(item.attr, sqparse2.NameX):
            raise NotImplementedError # todo
          elif isinstance(item.attr, sqparse2.AsterX):
            cols.extend(table2fields[item.parent.name])
          else:
            raise TypeError('attr_unk_type', type(item.attr))
        elif isinstance(item, sqparse2.NameX):
          matching_fields = [_f for _f in (next((field for field in table2fields[tab] if field.name == item.name), None) for tab in table_order) if _f]
          if len(matching_fields) != 1:
            raise sqparse2.SQLSyntaxError('missing_or_dupe_field', item, matching_fields)
          cols.append(matching_fields[0])
        elif isinstance(item, sqparse2.AliasX):
          cols.append(sqparse2.ColX(item.alias, None, None, None, None, None))
        else:
          raise TypeError('unk_item_type', type(item)) # pragma: no cover
    else:
      raise TypeError('unk_col_type', type(col)) # pragma: no cover
  return cols
...
class NameIndexer:
  ...
  """
  @staticmethod
  def update_aliases(aliases, aonly, item):
    "helper for ctor. takes AliasX or string as second arg"
    if isinstance(item, str):
      aliases[item] = item
    elif isinstance(item, sqparse2.AliasX):
      if not isinstance(item.alias, str):
        raise TypeError('alias not string', type(item.alias))
      if isinstance(item.name, sqparse2.NameX):
        aliases.update({item.alias:item.name.name, item.name.name:item.name.name})
      elif isinstance(item.name, sqparse2.SelectX):
        aliases.update({item.alias:item.alias})
        aonly[item.alias] = item.name
      else:
        raise TypeError('aliasx_unk_thing', type(item.name)) # pragma: no cover
    else:
      raise TypeError(type(item)) # pragma: no cover

  @classmethod
  def ctor_fromlist(cls, fromlistx):
    aliases = {}
    aonly = {}
    for from_item in fromlistx:
      if isinstance(from_item, str):
        cls.update_aliases(aliases, aonly, from_item)
      elif isinstance(from_item, sqparse2.AliasX):
        cls.update_aliases(aliases, aonly, from_item)
      elif isinstance(from_item, sqparse2.JoinX):
        cls.update_aliases(aliases, aonly, from_item.a)
        cls.update_aliases(aliases, aonly, from_item.b)
      else:
        raise TypeError(type(from_item)) # pragma: no cover
    table_order = sorted(set(aliases.values()))
    return cls(aliases, table_order, aonly)

  @classmethod
  def ctor_name(cls, name):
  ...
  def index_tuple(self, tables_dict, index, is_set):
    ...
    if not self.aonly_resolved:
      raise RuntimeError('resolve_aonly() before querying nix')
    with tables_dict.tempkeys():
      ...
      if isinstance(index, str):
        ...
        if len(candidates) != 1:
          raise ColumnNameError(("ambiguous_column" if candidates else "no_such_column"), index)
        tname, = candidates
        return self.table_order.index(tname), tables_dict[tname].lookup(index).index
      elif isinstance(index, sqparse2.AttrX):
        if index.parent.name not in self.aliases:
          raise TableNameError('table_notin_x', index.parent, self.aliases)
        tname = self.aliases[index.parent.name]
        tindex = self.table_order.index(tname)
        if isinstance(index.attr, sqparse2.AsterX):
          if is_set:
            raise ValueError('cant_set_asterisk') # todo: better error class
          else:
            return (tindex, )
        else:
          return (tindex, tables_dict[tname].lookup(index.attr).index)
        # todo: stronger typing here. make sure both fields of the AttrX are always strings.
      else:
        raise TypeError(type(index)) # pragma: no cover
  def rowget(self, tables_dict, row_list, index):
    ...
    for i in self.index_tuple(tables_dict, index, False):
      tmp = tmp[i]
    return tmp
  def rowset(self, tables_dict, row_list, index):
    raise NotImplementedError # note: shouldn't need this until update uses this
  def __repr__(self):
    return '<NameIndexer %s>'%self.table_order
...
def collapse_group_expr(groupx, cols, ret_row):
  ...
  for i, col in enumerate(cols.children):
    if col == groupx:
      ret_row[i] = ret_row[i][0]
  return ret_row

def run_select(ex, tables, table_ctor):
  nix, where = decompose_select(ex)
  nix.resolve_aonly(tables, table_ctor)
  with tables.tempkeys():
    # so aliases are temporary. todo doc: why am I doing this here *and* in index_tuple?
    ...
    if ex.group:
      ...
      if badcols:
        raise ValueError('illegal_cols_in_group', badcols)
      if contains(ex.cols, returns_rows):
        raise NotImplementedError('todo: unnest with grouping')
...
def starlike(tok):
  ...
  return isinstance(tok, sqparse2.AsterX) or isinstance(tok, sqparse2.AttrX) and isinstance(tok.attr, sqparse2.AsterX)
...
class Evaluator:
  ...
  def eval_agg_call(self, exp):
    ...
    if not isinstance(self.c_row, list):
  ...
  def eval_nonagg_call(self, exp):
    ...
    if exp.f == 'coalesce':
      left, right = args # todo: does coalesce take more than 2 args?
      return right if left is None else left
    elif exp.f == 'unnest':
      return self.eval(exp.args)[0] # note: run_select does some work in this case too
    elif exp.f in ('to_tsquery', 'to_tsvector'):
      return set(self.eval(exp.args.children[0]).split())
    else:
      raise NotImplementedError('unk_function', exp.f) # pragma: no cover
  ...
  def eval_unx(self, exp):
    ...
    if exp.op.op == '+':
      ...
      return -inner # pylint: disable=invalid-unary-operand-type
    elif exp.op.op == 'not':
      return threevl.ThreeVL.nein(inner)
    else:
      raise NotImplementedError('unk_op', exp.op) # pragma: no cover
  ...
  def eval(self, exp):
    ...
    if isinstance(exp, sqparse2.BinX):
      return evalop(exp.op.op, *list(map(self.eval, (exp.left, exp.right))))
    ...
    elif isinstance(exp, sqparse2.ArrayLit):
      return list(map(self.eval, exp.vals))
    elif isinstance(exp, (sqparse2.Literal, sqparse2.ArrayLit)):
      return exp.toliteral()
      ...
      for case in exp.cases:
        if self.eval(case.when):
          return self.eval(case.then)
      return self.eval(exp.elsex)
    elif isinstance(exp, sqparse2.CastX):
      if exp.to_type.type.lower() in ('text', 'varchar'):
        return str(self.eval(exp.expr))
      else:
        raise NotImplementedError('unhandled_cast_type', exp.to_type)
    ...
    # todo: why tuple, list, dict below? throw some asserts in here and see where these are coming from.
      ...
      return tuple(map(self.eval, exp))
    elif isinstance(exp, list):
      return list(map(self.eval, exp))
    elif isinstance(exp, dict):
      return exp
    elif isinstance(exp, sqparse2.NullX):
      return None
    ...
    elif isinstance(exp, sqparse2.AliasX):
      return self.eval(exp.name) # todo: rename AliasX 'name' to 'expr'
    else:
      raise NotImplementedError(type(exp), exp) # pragma: no cover
...
def depth_first_sub(expr, values):
  ...
  if len(arr) != len(values):
    raise ValueError('len', len(arr), len(values))
  for path, val in zip(arr, values):
    # todo: does ArrayLit get us anything? tree traversal?
    if isinstance(val, (str, int, float, type(None), dict)):
      expr[path] = sqparse2.Literal(val)
    elif isinstance(val, (list, tuple)):
      expr[path] = sqparse2.ArrayLit(val)
    else:
      raise TypeError('unk_sub_type', type(val), val) # pragma: no cover
  return expr

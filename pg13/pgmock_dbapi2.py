...
class STRING(PGMockType):
  pass

class BINARY(PGMockType):
  pass

class NUMBER(PGMockType):
  pass

class DATETIME(PGMockType):
  pass

class ROWID(PGMockType):
  pass
...
def expression_type(con, topx, ex):
  ...
  if isinstance(ex, sqparse2.Literal):
    if isinstance(ex.val, str):
      return STRING
    else:
      raise NotImplementedError('literal', type(ex.val))
...
def description_from_colx(con, ex, colx):
  if isinstance(colx, sqparse2.AliasX):
    ...
    raise NotImplementedError(ex) # probably math expressions and anonymous fields
...
class Cursor(pg.Cursor):
  ...
  def description(self):
    ...
    if self.lastx is None:
      return None
    if not isinstance(self.lastx, (sqparse2.SelectX, sqparse2.UpdateX, sqparse2.InsertX)):
      return None
    if isinstance(self.lastx, (sqparse2.UpdateX, sqparse2.InsertX)) and self.lastx.ret is None:
      return None
    # at this point we know this is an operation that returns rows
    if isinstance(self.lastx, (sqparse2.UpdateX, sqparse2.InsertX)):
  ...
  def callproc(self, procname, parameters=None):
    raise NotImplementedError("pgmock doesn't support stored procs yet")
  def __del__(self):
    self.close()
  def close(self):
    pass # for now pgmock doesn't have cursor resources to close
  ...
  def fetchone(self):
    ret = self.rows[self.rownumber]
    self.rownumber += 1
    return ret
  def fetchmany(self, size=None):
    raise NotImplementedError # hoping nobody cares about this one
  def fetchall(self):
    if self.rows is None:
      raise Error('no query to fetch')
    ret, self.rows = self.rows, None
    return ret
  def nextset(self):
    raise NotImplementedError('are we supporting multi result sets?')
...
def open_only(f):
  "decorator -- raises an error if the function is called with self.closed == True"
  @functools.wraps(f)
...
class Connection:
  ...
  def autocommit(self, value):
    if value and self.transaction_open:
      raise OperationalError("can't set autocommit with open transaction")
  ...
  def begin(self):
    if self.transaction_open:
      raise OperationalError("can't begin() with transaction_open")
  ...
  def commit(self):
    if not self.transaction_open:
      raise OperationalError("can't commit without transaction_open")
  ...
  def rollback(self):
    if not self.transaction_open:
      raise OperationalError("can't rollback without transaction_open")
